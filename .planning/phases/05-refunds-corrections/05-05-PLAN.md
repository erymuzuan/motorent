...
must_haves:
  ...
  key_links:
    ...
    - from: "Till.razor"
      ...
      via: "IModalService.Show"
      pattern: "ModalService.Show.*OverpaymentRefundDialog"
    - from: "Till.razor"
...
<task type="auto">
  <name>Task 2: Implement void workflow methods with proper PIN verification</name>
  <files>src/MotoRent.Client/Pages/Staff/Till.razor</files>
  <action>
Add the void workflow methods to the @code section. Note: VerifyManagerPinAsync is no longer needed because ManagerPinDialog now handles verification internally using the loaded manager.
...
{
    // No callback needed - ManagerPinDialog verifies PIN internally
...
**Important notes:**
- ManagerPinDialog now loads managers and verifies PIN internally (per 05-03 update)
- No OnVerify callback needed - just show the dialog
...
```razor
@inherits LocalizedModalBase<OverpaymentRefundResult, OverpaymentRefundDialog>
...
@code {
    ...
    private void OnConfirmRefund()
    {
        ...
        var result = new OverpaymentRefundResult
        {
            ...
            RentalId = RentalId,
            OriginalPaymentIds = OriginalPayments?.Select(p => p.PaymentId).Where(id => id > 0).ToList()
        };
...
public class OverpaymentRefundResult
{
    ...
    public List<int>? OriginalPaymentIds { get; set; }
}
```
...
Add a "Refund" action button to transactions that have overpayment. This could be:
1. A separate button on RentalPayment transactions where overpayment detected
2. Or accessible via transaction details/context menu
...
private async Task InitiateOverpaymentRefundAsync(TillTransaction txn)
{
    ...
    var refundParams = new DialogParameters
    {
        { nameof(OverpaymentRefundDialog.OriginalPayments), payments },
        { nameof(OverpaymentRefundDialog.TotalPaidThb), totalPaid },
        { nameof(OverpaymentRefundDialog.AmountDueThb), amountDue },
        { nameof(OverpaymentRefundDialog.RentalId), txn.RentalId },
        { nameof(OverpaymentRefundDialog.TillThbBalance), m_session.GetCurrencyBalance(SupportedCurrencies.THB) }
    };

    var refundOptions = new DialogOptions { CloseButton = true, MaxWidth = MaxWidth.Small };
    var refundResult = await ModalService.Show<OverpaymentRefundDialog>(
        Localizer["OverpaymentRefund"],
        refundParams,
        refundOptions).Result;

    if (refundResult.Canceled) return;
