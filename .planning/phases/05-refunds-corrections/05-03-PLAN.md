...
# Dialog base class pattern (use object for generic TEntity)
...
**IMPORTANT:** Use `LocalizedDialogBase<object, ManagerPinDialog>` (not `LocalizedModalBase<string, ...>`) because:
1. The base class requires TEntity to have a `new()` constraint
2. `string` does not satisfy `new()` constraint
3. Follow the TillTransactionDialog pattern which uses `object` for dialogs that don't edit an entity
    ...
    private async Task VerifyPinAsync()
    {
        ...
        try
        {
            ...
            if (result.IsValid)
            {
                ...
                DialogService.Close(m_selectedManagerUserName);
                return;
            }
...
- Uses `LocalizedDialogBase<object, ManagerPinDialog>` (matching TillTransactionDialog pattern)
...
- Verifies PIN directly via ManagerPinService.VerifyPin (no callback needed)
- Returns manager username on success via DialogService.Close
- Uses DialogService.Close (not ModalService.Close) per codebase pattern
  </action>
