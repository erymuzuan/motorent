    ...
    else
    {
        ...
        @if (m_availableManagers.Count > 1)
        {
            <div class="manager-select mt-3">
                ...
                <select class="form-select" @bind="m_selectedManagerUserName" disabled="@m_verifying">
...
@code {
    ...
    {
        // Load managers who have PIN configured
        // For now, use the logged-in user's account to filter managers
        // This would typically query users in ManagementRoles with CanApproveVoids = true
        // TODO: Implement proper manager loading based on current shop/org
        m_availableManagers = []; // Will be populated by service
    ...
    {
        if (string.IsNullOrEmpty(m_selectedManagerUserName))
        {
        ...
        {
            // Load manager user
            // TODO: Inject CoreDataContext and load user
            // For now, verification is handled by callback pattern

            // This dialog works with a verification callback
            // The parent component provides the manager and verifies
            if (OnVerify != null)
            {
                var result = await OnVerify.Invoke(m_selectedManagerUserName, m_pin);
                if (result.IsValid)
                {
                    // Success - close dialog with manager username
    ...
    /// <summary>
    /// Callback to verify PIN. Parent component provides this.
...
- OnVerify callback allows parent to handle actual verification
- Returns manager username on success via ModalResult.Ok
  ...
  <done>ManagerPinDialog.razor exists with PIN keypad, dots display, lockout handling</done>
</task>
