...
public class TillService(RentalDataContext context, ExchangeRateService exchangeRateService)
{
        ...
        string username)
    {
        ...
        var transaction = new TillTransaction
        {
            ...
            TransactionTime = DateTimeOffset.Now,
            RecordedByUserName = username,
            Notes = originalPaymentIds != null && originalPaymentIds.Count > 0
    ...
    #region Denomination Count Operations
        ...
        string? notes = null)
    {
        ...
        var session = await GetSessionByIdAsync(tillSessionId);
        if (session == null)
            return SubmitOperation.CreateFailure("Session not found");
        ...
        foreach (var breakdown in breakdowns)
        {
            ...
            else
            {
                ...
                var conversion = await m_exchangeRateService.ConvertToThbAsync(breakdown.Currency, breakdown.Total);
                if (conversion != null)
                {
        ...
        var existingCount = await GetDenominationCountAsync(tillSessionId, countType, includeDrafts: true);

        TillDenominationCount denominationCount;
        if (existingCount != null && !existingCount.IsFinal)
        {
        ...
        else if (existingCount != null && existingCount.IsFinal)
        {
        ...
        using var persistenceSession = Context.OpenSession(countedByUserName);
    ...
    {
        var query = Context.CreateQuery<TillDenominationCount>()
            .Where(d => d.TillSessionId == tillSessionId)
            .Where(d => d.CountType == countType);
    ...
    {
        var result = await Context.LoadAsync(
            Context.CreateQuery<TillDenominationCount>()
                .Where(d => d.TillSessionId == tillSessionId)
    ...
    /// </summary>
    private decimal GetExpectedBalanceForCurrency(TillSession session, string currency)
    {
        return session.GetCurrencyBalance(currency);
    }
