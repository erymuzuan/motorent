-- Migration: Create Feedback table in Core schema
-- Date: 2026-02-12

IF NOT EXISTS (SELECT * FROM sys.tables WHERE name = 'Feedback' AND schema_id = SCHEMA_ID('Core'))
BEGIN
    ...
    PRINT 'Created [Core].[Feedback] table with indexes'
END
ELSE
BEGIN
    PRINT '[Core].[Feedback] table already exists'
END
